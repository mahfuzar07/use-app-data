...
// src/context.tsx
var AppDataContext = React.createContext(null);
function AppDataProvider({ api, authApi, children }) {
  const value = React__default.default.useMemo(() => ({ api, authApi }), [api, authApi]);
  return /* @__PURE__ */ jsxRuntime.jsx(AppDataContext.Provider, { value, children });
}
...
function useAppData(options) {
  ...
  const queryKey = React.useMemo(() => normalizeKey(key), [key]);
  const headersJson = JSON.stringify(extraHeaders != null ? extraHeaders : {});
  const stableHeaders = React.useMemo(() => extraHeaders != null ? extraHeaders : {}, [headersJson]);
  ...
  const mutation = reactQuery.useMutation({
    ...
    onError: (err, variables, context) => {
      ...
      const error = err instanceof axios.AxiosError ? err : new axios.AxiosError("Unknown error occurred");
